...
from kb_network_agent.models import (
    TelemetryData, GPUInfo, DiskInfo, DockerContainer, DockerStats,
    OllamaModel, OllamaStats, DatabaseStatus, DatabaseStats,
    PendingUpdate, SystemUpdates
)
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row_id, data_str in rows:
                    try:
                        cached.append((row_id, TelemetryData.model_validate_json(data_str)))
                    except Exception as parse_err:
                        logger.error(f"Failed to parse cached telemetry ID {row_id}: {parse_err}")
                        # delete bad data
                        conn.execute("DELETE FROM telemetry_cache WHERE id = ?", (row_id,))
                
                # Delete rows that were successfully loaded
                if cached:
                    ids = [c[0] for c in cached]
                    conn.execute(f"DELETE FROM telemetry_cache WHERE id IN ({','.join('?' for _ in ids)})", ids)
                    conn.commit()
        except Exception as e:
...
def get_dns_servers() -> List[str]:
    ...
    if sys.platform == "win32":
        try:
            ...
            res = subprocess.run(["powershell", "-Command", cmd], capture_output=True, text=True, shell=True)
            dns_servers = list(set([line.strip() for line in res.stdout.strip().split("\n") if line.strip()]))
        except Exception as e:
...
def collect_gpu_info() -> Optional[GPUInfo]:
    ...
    if shutil.which("nvidia-smi"):
        try:
            res = subprocess.run(
                ["nvidia-smi", "--query-gpu=gpu_name,driver_version,memory.total,memory.used", "--format=csv,noheader,nounits"],
    ...
    if sys.platform == "win32":
        ...
        if shutil.which("lshw"):
            try:
                res = subprocess.run(["lshw", "-c", "display"], capture_output=True, text=True)
            ...
            try:
                ...
                for line in res.stdout.strip().split("\n"):
                    if "upgradable from:" in line:
                        ...
                        old_version = None
                        if "upgradable from:" in line:
                            old_version = line.split("upgradable from:")[-1].replace("]", "").strip()
...
def collect_telemetry() -> TelemetryData:
    ...
    if sys.platform == "win32":
        os_name = "Windows"
        os_version = f"Windows {platform_version()}" if shutil.which("powershell") else sys.getwindowsversion().service_pack or "Windows"
...
def platform_version() -> str:
    try:
        res = subprocess.run(["powershell", "[System.Environment]::OSVersion.Version.ToString()"], capture_output=True, text=True, shell=True)
...
def send_telemetry_to_server(server_url: str, token: str, data: TelemetryData) -> bool:
    try:
        ...
        r = httpx.post(f"{server_url}/telemetry", json=data.model_dump(), headers=headers, timeout=5.0)
